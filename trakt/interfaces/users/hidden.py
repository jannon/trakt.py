...
class UsersHiddenInterface(Interface):
    ...
    def get(self, section, type=None, page=None, per_page=None, **kwargs):
        ...
        response = self.http.get(
            ...
            query={"type": type, "page": page, "limit": per_page},
            **dictfilter(kwargs, pop=["authenticated", "pagination", "validate_token"]),
        )
        ...
        if isinstance(items, PaginationIterator):
            return items.with_mapper(
                lambda items: HiddenItemMapper.hidden_items(self.client, items)
            )
